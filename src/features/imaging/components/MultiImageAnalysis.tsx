    ...
    >
      ...
      >
        <div className="space-y-2 pt-3">
          {images.map((img, i) => {
            const colorCls = MODALITY_COLOR[img.modality] ?? 'from-slate-500/20 to-slate-600/10 border-slate-500/30 text-slate-400';
            ...
            return (
              <motion.button
                ...
                className={`w-full flex items-start gap-3 p-3 rounded-xl border bg-gradient-to-r transition-all text-left
                  ${colorCls}
                  ${isActive ? 'ring-1 ring-white/15' : 'hover:brightness-110'}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isActive && (
                      <span className="text-[9px] font-semibold text-white bg-blue-600 px-1.5 py-0.5 rounded-full">Active</span>
                    )}
                  </div>
